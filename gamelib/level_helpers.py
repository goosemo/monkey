...
def make_chain(entity_manager, pos, num_links = 10, mass = 1, **kwargs):
    ...
    for i in range(num_links):
        entity = world.BaseEntity((x +i*(CHAIN_LINK_LEN+2), y), CHAIN_LINK_POLY, mass, grabable=True, taggable = False, friction=0.1, texture_name="chain")
        links.append(entity)
...
def MovableBox(pos, width=50, height=50, mass=5, **kwargs):
    ...
    return world.BaseEntity(pos, verts, mass, dynamic=True, texture_name="crate1", **kwargs)

def FloorBox(pos, width=100, height=40, **kwargs):
    verts = [(0, -height),(0,0),(width,0),(width, -height)]
    return world.BaseEntity(pos, verts, pymunk.inf, dynamic=False, texture_name="floorBox", **kwargs)

def MastBox(pos, width=40, height=200, **kwargs):
    verts = [(0, -height),(0,0),(width,0),(width, -height)]
    return world.BaseEntity(pos, verts, pymunk.inf, dynamic=False, texture_name="vertPole", **kwargs)
